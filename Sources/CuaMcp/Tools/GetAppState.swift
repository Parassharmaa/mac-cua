...
extension Tools {
    static func getAppState(app bundleIdOrName: String) throws -> (
        app: NSRunningApplication, root: AXNode, text: String
    ) {
        guard Permissions.axTrusted() else {
            throw MCPError(
                code: -32000,
                message:
                    "Accessibility permission not granted. Grant it in System Settings → Privacy & Security → Accessibility."
            )
        }
        ...
        guard let app = matched else {
            throw MCPError(
                code: -32000, message: "Running application not found: \(bundleIdOrName)")
        }
